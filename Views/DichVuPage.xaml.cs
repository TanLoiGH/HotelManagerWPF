...
{
    private List<DichVuRow> _allDv = [];
    private DichVuRow? _selected;
    private bool _isNew;
    ...
    {
        if (DvGrid.SelectedItem is not DichVuRow row) return;
        _selected = row;
        _isNew = false;
        ShowForm(row);
    }
    ...
    private void ShowForm(DichVuRow? row)
    {
