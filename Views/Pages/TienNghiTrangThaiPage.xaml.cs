...
public partial class TienNghiTrangThaiPage : Page
{
    public TienNghiTrangThaiPage()
    {
        InitializeComponent();
    ...
    private async void BtnCapNhat_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            using var db = new QuanLyKhachSanContext();
            var svc = new TienNghiService(db);
            await svc.CapNhatTrangThaiAsync(maPhong, item.MaTienNghi, maTrangThai, isThayMoi: false);
    ...
    private async void TienNghiGrid_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        using var db = new QuanLyKhachSanContext();
        var svc = new TienNghiService(db);
        await svc.CapNhatTrangThaiAsync(maPhong, item.MaTienNghi, "TNTT02", isThayMoi: false);
        CboPhong_SelectionChanged(CboPhong, null!);
    }
    ...
    private async void BtnGhiXuLy_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            await tienNghiSvc.CapNhatTrangThaiAsync(maPhong, item.MaTienNghi, "TNTT01", isThayMoi: false);
